...
bool DIM::_erase_node(ULL at, int vv) {
    ...
    for (; !Q.empty();) {
        ...
        for (auto it = lower_bound(hs[at].par.begin(), hs[at].par.end(),
                                   make_pair(v, 0));
             it != hs[at].par.end() && it->first == v; it++) {
            Q.push(it->second);
            tree[it->second] = v;
        }
    }
        ...
        for (auto jt = ps.lower_bound(make_pair(v, 0));
            ...
            if (binary_search(hs[at].x.begin(), hs[at].x.end(), re)) {
                if (!tree.count(w) && hs[at].H.count(w)) {
                    ...
                    auto kt2 =
                        lower_bound(hs[at].par.begin(), hs[at].par.end(), re);
    ...
    for (; !Q.empty();) {
        ...
        Q.pop();
        for (auto it =
                 lower_bound(hs[at].x.begin(), hs[at].x.end(), make_pair(v, 0));
             it != hs[at].x.end() && it->first == v; it++) {
...
// error = O(n*2^-32)
inline int DIM::Xorshift::gen_int(int n) { return (int)(n * gen_double()); }
